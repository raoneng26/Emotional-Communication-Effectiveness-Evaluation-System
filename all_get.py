...
# s = current_directory+r"\chromedriver.exe"
# chrome_path = s
# s = Service(s)#----------------

# current_directory = os.getcwd()   #-------------------
# s = current_directory+r"code/chrome-win64/chromedriver.exe" 
# s = Service(s)  # -----------------
...
class GetBiliInfo:
    def __init__(self, title, view_number, post_name, post_time, barrage_number, view_like, view_coin,
        ...
        self.headers = {
            ...
            'Cookie': (
                "buvid3=EF6E53D6-2137-6FDC-CE79-D53360666F9C06101infoc; "
                "b_nut=1701178206; "
                "b_lsid=AAC7821B_18C161F6927; "
                "_uuid=310A10810DB-53AA-1036C-87DB-2C173942737906511infoc; "
                "buvid4=BDA1F8D7-8F78-0624-B550-3241BA90BAD306837-023112813-; "
                "buvid_fp=331dee6fd05086062a753a166945b8b5; "
                "bili_ticket=eyJhbGciOiJIUzI1NiIsImtpZCI6InMwMyIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MDE0Mzc0MDksImlhdCI6MTcwMTE3ODE0OSwicGx0IjotMX0.vMkVHyeqtL4aeKo39XOxzq6Vt0SQPOMFy1jgUKL7Fgw; "
                "bili_ticket_expires=1701437349; "
                "SESSDATA=7d5a5855%2C1716732239%2C70ae1%2Ab1CjBtKmy5w-R19jc2jDFtw21m-4mkd9hZMiaBawR5ntZW3grbDF-cTRPO5S9zDcRYb9oSVnN6cjRlSVVETWREYkpfeFJ2T20wbTN6TnRxZ3dNVENSM0huOV9PNHc5WFRNUWlDMHJfUVRxbEZXOEZ5TkdiYjJvSDJFcXBiNVFQaktpUVowS0xHRjd3IIEC; "
                "bili_jct=dab72fe419105387da40eef79a337dfb; "
                "DedeUserID=1789841674; "
                "DedeUserID__ckMd5=a0fbbc542506eba3; "
                "sid=nr3q39ob"
            )
        }
    ...
    def second_comment(self, temp, root_id):
        print(f'https://api.bilibili.com/x/v2/reply/reply?jsonp=jsonp&pn={temp}&type=1&oid={self.av_num}&ps=10&root={root_id}')
    ...
    def first_comment(self, w1):
        print("在这里：",f'https://api.bilibili.com/x/v2/reply/main?jsonp=jsonp&next={w1}&type=1&oid={self.av_num}&mode=3&plat=1')
...
def kuaishou(writer, url1):
    ...
    time.sleep(2)
    # 标题
    title = driver.find_element(By.XPATH,"//p[@class='video-info-title']").text
    while title == '':
        print('请检查网页状态')
        driver.implicitly_wait(10)
        title = driver.find_element(By.XPATH,"//p[@class='video-info-title']").text
    # 发布者
    post_name = driver.find_element(By.XPATH,"//span[@class='profile-user-name-title']").text
    # 发布时间
    post_time = driver.find_element(By.XPATH,"//span[@class='photo-time']").text
...
def weibo(writer, url1, num1,key):
    if num1 != 2:
        try:
            ...
            # publisher = driver.find_element(By.XPATH, "//*[@id='pl_feedlist_index']/div[2]/div[1]/div[2]/div[1]/div[2]/div[1]/div[2]/a").text # ----------
            ...
            # transmit = driver.find_element(By.XPATH,
            #                                "//*[@id='pl_feedlist_index']/div[2]/div[1]/div[2]/div[2]/ul/li[1]/a").text #---------------------
            res1 = re.findall(r'\d', transmit)
            transmit_count = ''.join(res1)
            if transmit_count == '':
                transmit_count = 0
            ...
            # comment = driver.find_element(By.XPATH,
            #                             "//*[@id='pl_feedlist_index']/div[2]/div[1]/div[2]/div[2]/ul/li[2]/a").text #-----------------------
            res2 = re.findall(r'\d', comment)
            comment_count = ''.join(res2)
            if comment_count == '':
                comment_count = 0
            ...
            # like = driver.find_element(By.XPATH,
            #                            "//*[@id='pl_feedlist_index']/div[2]/div[1]/div[2]/div[2]/ul/li[3]/a/button/span[2]") #-------------------
            res3 = re.findall(r'\d', like)
            like_count = ''.join(res3)
            if like_count == '':
                like_count = 0
            ...
            #帖子账号粉丝数
            # fans = driver.find_element(By.XPATH, "//div[@class='f14 cla']/div").text  #-------原代码-------
...
def bilibili(writer, url1):
    ...
    # title
    title = driver.find_element(By.XPATH,"//h1").get_attribute('title')
    # 视频播放量#viewbox_report > div > div > span.view.item
    view = driver.find_element(By.XPATH,"//span[@class='view item']").text
    view_number1 = re.findall(r'\d', view)
    view_number = ''.join(view_number1)
    # 发布者
    try:
        post_name = driver.find_element(By.CSS_SELECTOR,"a.up-name").text
    except:
        post_name = driver.find_element(By.CSS_SELECTOR,"div.up-panel-container > div.members-info-container > div > div.container > div:nth-child(1) > div > div > a").text
    # print(post_name)
    # 发布时间
    post_time = driver.find_element(By.CSS_SELECTOR,"span.pubdate-ip.item").text
    # print(post_time)
    # 弹幕数量
    barrage = driver.find_element(By.CSS_SELECTOR,"span.dm.item").text
    res = re.findall(r'\d', barrage)
    barrage_number = ''.join(res)
    # print(barrage_number)
    # 视频点赞数
    view_like1 = driver.find_element(By.CSS_SELECTOR,".video-like-info").text
    if '万' in view_like1:
        view_like = int(float(view_like1.replace('万', '')) * 10000)
    else:
        view_like = int((view_like1))
    # print(view_like)
    # 视频投币
    view_coin1 = driver.find_element(By.CSS_SELECTOR,".video-coin-info").text
    if '万' in view_coin1:
        view_coin = int(float(view_coin1.replace('万', '')) * 10000)
    else:
        view_coin = int((view_coin1))
    # print(view_coin)
    # 视频收藏
    view_collect1 = driver.find_element(By.CSS_SELECTOR,".video-fav-info").text
    if '万' in view_collect1:
        view_collect = int(float(view_collect1.replace('万', '')) * 10000)
    else:
        view_collect = int((view_collect1))
    # print(view_collect)
    # 视频分享
    view_share1 = driver.find_element(By.CSS_SELECTOR,".video-share-info").text
    if '万' in view_share1:
        view_share = int(float(view_share1.replace('万', '')) * 10000)
    else:
        view_share = int((view_share1))
    # print(view_share)
    # 评论数量
    comment_number = driver.find_element(By.CSS_SELECTOR,'.reply-header').text
    comment_number = re.findall(r'\d+', comment_number)[0]
